...
static TimerTime_t gTxWatchdogStart = 0;
...
static void HandleRadioRxErrorTimeout( LoRaMacEventInfoStatus_t rx1EventInfoStatus, LoRaMacEventInfoStatus_t rx2EventInfoStatus )
{
    ...
    if( classBRx == false )
    {
        if( MacCtx.RxSlot == RX_SLOT_WIN_1 )
        {
            ...
            // MacDone when Win2 window has already passed, OR when Win2 timer
            // is no longer active (it fired but returned early due to race
            // condition where RxSlot was still WIN_1 at the time it fired).
            if( ( TimerGetElapsedTime( Nvm.MacGroup1.LastTxDoneTime ) >= MacCtx.RxWindow2Delay ) ||
                ( TimerIsStarted( &MacCtx.RxWindowTimer2 ) == false ) )
            {
                TimerStop( &MacCtx.RxWindowTimer2 );
                MacCtx.MacFlags.Bits.MacDone = 1;
            }
...
static void LoRaMacHandleMlmeRequest( void )
{
    bool joinReqActive = false;

    joinReqActive = ( LoRaMacConfirmQueueIsCmdActive( MLME_JOIN ) == true ) ||
                    ( LoRaMacConfirmQueueIsCmdActive( MLME_REJOIN_0 ) == true ) ||
                    ( LoRaMacConfirmQueueIsCmdActive( MLME_REJOIN_1 ) == true ) ||
                    ( LoRaMacConfirmQueueIsCmdActive( MLME_REJOIN_2 ) == true );

    // Handle join request
    if( ( MacCtx.MacFlags.Bits.MlmeReq == 1 ) || ( joinReqActive == true ) )
    {
        if( joinReqActive == true )
        {
...
void LoRaMacProcess( void )
{
    ...
    {
        if( ( gTxWatchdogStart != 0 ) && ( TimerGetElapsedTime( gTxWatchdogStart ) > 7000U ) )
        {
            gTxWatchdogStart = TimerGetCurrentTime( );
            LoRaMacRadioEvents.Events.TxTimeout = 1;
        }
    }
